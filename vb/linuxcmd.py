...
mkdir dir1 dir2 :한번에 여러 디렉터리 생성
...
6    110 rw- 읽기 쓰기권한있음
7    111 rwx  모든 권한 있음
예를 들어 '777'의 경우이진수로 11111111이고rwxrwxrwx라는 의미를 가지므로
...
cp -4 dir1 dir2 : 디렉터리 복사 폴더안의 모든 하위 경로와 파일들을 복사
...
rm -r dir : dir 디렉터리 삭제 (디렉터리는 -r 옵션없이 삭제불가)


8. cat (catenote)  : 파일의 내용을 화면에 출력, 리다이렉션 기로('>')를 사용하여 새로운 파일생성
...
10. head, tail - 파일위(head) 아래(tail) 출력
...
tail -n 5 ./test.log #5번째줄부터 끝까지 출력
tail -f ./test.log #추가되는 내용 oppend하여 출력됨
